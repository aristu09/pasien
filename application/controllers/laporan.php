...
class laporan extends CI_controller
{
	...
	public function antrian($value='')
	{
		$antrian=$this->m_antrian->antrian();
		$no_antrian=$antrian->no_antrian;
		$no_urut=substr($no_antrian, 8,4);
		$tanggal=substr($no_antrian, 0,8);
		$tgl=date('Ymd');
		if ($tanggal==$tgl) {
			$no_urut++;
		}else{
		...
		$no_antrian=$tgl.sprintf('%04s',$no_urut);
		return $no_antrian;
	}
